...
//Make a list with all the available MEGA2560 timers.
//Timer::Ptr timer[6];
Timer::Ptr timer = new Timer8(t_alias::T0);
...
void setup()
{
	...
	//TIMER0 SETTINGS.
	timer->initialize(t_mode::CTC, t_interrupt::COMPA);		// TIMER0_OVF_vect not available (used by arduino).
	timer->setCompareValueA(200);
	timer->setPrescaler(1); 					// Available prescalers for T0: 1, 8, 64, 256, 1024.
	timer->reset();
...
{
	Serial.print("T0::");
	Serial.println(timer->getOverflows());
	/*Serial.print("\tT1::");
