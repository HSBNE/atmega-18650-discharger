...
typedef std::function<uint32_t(void)> time_get;
#else
typedef void (*timer_callback)();
typedef uint32_t (*time_get)();
#endif // __AVR__
...
public:
    ...
    // call function f every d milliseconds
    int setInterval(unsigned long d, timer_callback f, time_get g);

    // call function f once after d milliseconds
    int setTimeout(unsigned long d, timer_callback f);

    // call function f every d milliseconds for n times
    int setTimer(unsigned long d, timer_callback f, int n, time_get g);
...
private:
    ...
    // time function to use for this timer
    time_get timeFunctions[MAX_TIMERS];
